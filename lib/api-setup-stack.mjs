...
export class ApiSetupStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // /sessions/{session-id} DELETE endpoint
    // Define a path parameter under the existing sessionsResource
    const sessionResource = sessionsResource.addResource("{session-id}");
    const userSessionResource = sessionResource.addResource("{user-id}");
    const deleteSessionLambda = this.createDeleteSessionLambda(
      tutoringSessionsTable
    );

    userSessionResource.addMethod(
      "DELETE", // The DELETE method for deleting a session
      new apigateway.LambdaIntegration(deleteSessionLambda) // Lambda integration for session deletion
    );
  }
  ...
  // Create a Lambda function to handle session deletion
  createDeleteSessionLambda(tutoringSessionsTable) {
    const deleteSessionLambda = new lambda.Function(
      this,
      "DeleteSessionFunction",
      {
        runtime: lambda.Runtime.NODEJS_20_X,
        code: lambda.Code.fromAsset("lambda/deleteSession"), // Directory for the deleteSession Lambda
        handler: "index.handler",
        environment: {
          TUTORING_SESSIONS_TABLE: tutoringSessionsTable.tableName,
        },
    ...
    // Grant delete permissions on the DynamoDB table to the Lambda function
    tutoringSessionsTable.grantWriteData(deleteSessionLambda);
